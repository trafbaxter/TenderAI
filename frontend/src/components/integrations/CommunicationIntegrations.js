import React, { useState } from 'react';
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Badge } from "@/components/ui/badge";
import { Integration } from "@/entities/all";
import { MessageSquare, Plus, Settings, CheckCircle, AlertCircle } from "lucide-react";

export default function CommunicationIntegrations({ integrations, onUpdate }) {
  const [showAddForm, setShowAddForm] = useState(false);
  const [newIntegration, setNewIntegration] = useState({
    name: "",
    provider: "",
    config: {
      webhook_url: "",
      bot_token: "",
      channel: "",
      api_key: ""
    }
  });

  const communicationProviders = [
    { 
      id: "slack", 
      name: "Slack", 
      icon: "💬",
      description: "Send tender notifications to Slack channels"
    },
    { 
      id: "mattermost", 
      name: "Mattermost", 
      icon: "🗨️",
      description: "Integrate with Mattermost team chat"
    },
    { 
      id: "discord", 
      name: "Discord", 
      icon: "🎮",
      description: "Send updates to Discord servers"
    },
    { 
      id: "teams", 
      name: "Microsoft Teams", 
      icon: "👥",
      description: "Post to Teams channels"
    },
    { 
      id: "telegram", 
      name: "Telegram", 
      icon: "✈️",
      description: "Send notifications via Telegram bot"
    }
  ];

  const handleAddIntegration = async () => {
    try {
      await Integration.create({
        ...newIntegration,
        type: "communication",
        status: "pending"
      });
      setShowAddForm(false);
      setNewIntegration({
        name: "",
        provider: "",
        config: {
          webhook_url: "",
          bot_token: "",
          channel: "",
          api_key: ""
        }
      });
      onUpdate();
    } catch (error) {
      console.error("Error adding integration:", error);
    }
  };

  const testConnection = async (integration) => {
    console.log("Testing communication integration:", integration.name);
    await Integration.update(integration.id, { status: "active", last_sync: new Date().toISOString() });
    onUpdate();
  };

  return (
    <div className="space-y-6">
      
      {/* Available Communication Platforms */}
      <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <MessageSquare className="w-5 h-5" />
            Communication Integrations
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {communicationProviders.map((provider) => (
              <div
                key={provider.id}
                className="p-4 border-2 border-dashed border-slate-200 rounded-lg hover:border-blue-300 hover:bg-blue-50/50 transition-all cursor-pointer"
                onClick={() => {
                  setNewIntegration(prev => ({ 
                    ...prev, 
                    provider: provider.id, 
                    name: `${provider.name} Integration`
                  }));
                  setShowAddForm(true);
                }}
              >
                <div className="flex items-start gap-3">
                  <div className="text-2xl">{provider.icon}</div>
                  <div>
                    <h3 className="font-semibold text-slate-800">{provider.name}</h3>
                    <p className="text-sm text-slate-600">{provider.description}</p>
                  </div>
                </div>
              </div>
            ))}
          </div>
          
          <Button
            onClick={() => setShowAddForm(true)}
            className="w-full bg-gradient-to-r from-green-600 to-green-700"
          >
            <Plus className="w-4 h-4 mr-2" />
            Add Communication Integration
          </Button>
        </CardContent>
      </Card>

      {/* Add Integration Form */}
      {showAddForm && (
        <Card className="bg-white/95 backdrop-blur-sm border-0 shadow-xl">
          <CardHeader>
            <CardTitle>Add Communication Integration</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Integration Name</Label>
                <Input
                  value={newIntegration.name}
                  onChange={(e) => setNewIntegration(prev => ({ ...prev, name: e.target.value }))}
                  placeholder="Tender Notifications"
                />
              </div>
              <div>
                <Label>Platform</Label>
                <select
                  className="w-full p-2 border rounded-md"
                  value={newIntegration.provider}
                  onChange={(e) => setNewIntegration(prev => ({ ...prev, provider: e.target.value }))}
                >
                  <option value="">Select platform</option>
                  {communicationProviders.map((provider) => (
                    <option key={provider.id} value={provider.id}>
                      {provider.name}
                    </option>
                  ))}
                </select>
              </div>
            </div>

            {newIntegration.provider === 'slack' && (
              <>
                <div>
                  <Label>Webhook URL</Label>
                  <Input
                    value={newIntegration.config.webhook_url}
                    onChange={(e) => setNewIntegration(prev => ({
                      ...prev,
                      config: { ...prev.config, webhook_url: e.target.value }
                    }))}
                    placeholder="https://hooks.slack.com/services/..."
                  />
                </div>
                <div>
                  <Label>Channel</Label>
                  <Input
                    value={newIntegration.config.channel}
                    onChange={(e) => setNewIntegration(prev => ({
                      ...prev,
                      config: { ...prev.config, channel: e.target.value }
                    }))}
                    placeholder="#tenders"
                  />
                </div>
              </>
            )}

            {newIntegration.provider === 'teams' && (
              <div>
                <Label>Webhook URL</Label>
                <Input
                  value={newIntegration.config.webhook_url}
                  onChange={(e) => setNewIntegration(prev => ({
                    ...prev,
                    config: { ...prev.config, webhook_url: e.target.value }
                  }))}
                  placeholder="https://outlook.office.com/webhook/..."
                />
              </div>
            )}

            {newIntegration.provider === 'telegram' && (
              <>
                <div>
                  <Label>Bot Token</Label>
                  <Input
                    type="password"
                    value={newIntegration.config.bot_token}
                    onChange={(e) => setNewIntegration(prev => ({
                      ...prev,
                      config: { ...prev.config, bot_token: e.target.value }
                    }))}
                    placeholder="123456:ABC-DEF..."
                  />
                </div>
                <div>
                  <Label>Chat ID</Label>
                  <Input
                    value={newIntegration.config.channel}
                    onChange={(e) => setNewIntegration(prev => ({
                      ...prev,
                      config: { ...prev.config, channel: e.target.value }
                    }))}
                    placeholder="-1001234567890"
                  />
                </div>
              </>
            )}

            <div className="bg-green-50 p-4 rounded-lg">
              <h4 className="font-medium text-green-800 mb-2">Notification Types</h4>
              <ul className="text-sm text-green-700 space-y-1">
                <li>• New high-match tenders found</li>
                <li>• Tender deadline reminders</li>
                <li>• Bid submission confirmations</li>
                <li>• Email acknowledgments received</li>
                <li>• Meeting notifications</li>
              </ul>
            </div>

            <div className="flex justify-end gap-3">
              <Button variant="outline" onClick={() => setShowAddForm(false)}>
                Cancel
              </Button>
              <Button onClick={handleAddIntegration}>
                Add Integration
              </Button>
            </div>
          </CardContent>
        </Card>
      )}

      {/* Active Integrations */}
      {integrations.length > 0 && (
        <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
          <CardHeader>
            <CardTitle>Active Communication Channels</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {integrations.map((integration) => (
                <div key={integration.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="text-xl">
                      {communicationProviders.find(p => p.id === integration.provider)?.icon || "💬"}
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-800">{integration.name}</h3>
                      <p className="text-sm text-slate-600">
                        {integration.provider} • {integration.config.channel || 'Default channel'}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant={integration.status === 'active' ? 'default' : 'secondary'}>
                      {integration.status === 'active' && <CheckCircle className="w-3 h-3 mr-1" />}
                      {integration.status === 'error' && <AlertCircle className="w-3 h-3 mr-1" />}
                      {integration.status}
                    </Badge>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => testConnection(integration)}
                    >
                      <Settings className="w-4 h-4 mr-2" />
                      Test
                    </Button>
                  </div>
                </div>
              ))}
            </div>
          </CardContent>
        </Card>
      )}
    </div>
  );
}